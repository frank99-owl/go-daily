...
并非所有题目都支持 AI 教练。题目必须通过两项检查：

1.  **审批 ID**：存在于 `coachEligibleIds.json` 中。
2.  **解说备注**：在用户当前语言下拥有有效的 `solutionNote`。
    如果用户在不支持的题目上点击教练，UI 将降级为“显示答案”模式。
