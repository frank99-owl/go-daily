...
**行为**：`GET` 是邮件底部可见链接，写入退订状态后带邮件状态参数跳回 `/en`。`POST` 用于邮件客户端通过 `List-Unsubscribe` 与 `List-Unsubscribe-Post` header 发起的 RFC 8058 一键退订，写入 `profiles.email_opt_out = true` 后返回空响应。
