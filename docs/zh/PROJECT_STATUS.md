...
- **当前执行项 P2-C 生产烟测**：本轮只补本地、dry-run、mock、静态分析或可控 HTTP smoke；不部署、不真实发送邮件、不真实创建 Stripe 支付、不访问或修改外部系统。
...
3. **生产烟测（P2-C）**：补齐本地 smoke test、性能预算、SEO、PWA/offline、错误体验、邮件 dry-run 与 Stripe 本地校验；所有检查必须可重复、可回滚，并避免外部副作用。
