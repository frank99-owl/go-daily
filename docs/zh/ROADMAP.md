...
P0-A / P0-B / P0-C / P0-D 已完成首轮落地，后续仍可按验证结果小步修正。

| 子项 | 目标                         | 交付                                                                       |
| ---- | ---------------------------- | -------------------------------------------------------------------------- |
| P0-A | 同步事实源，建立内容质量分层 | 更新项目状态、修正文档 drift、新增内容质量模型、落地 Coach 分层数据源      |
...
P2-A / P2-B / P2-C / P2-D / P2-E 已完成首轮落地。Phase 3 首轮完成；生产配置与发布窗口 smoke 已通过，后续进入 GitHub release、公开发布审批和真实用户验证阶段。

P2-C 已补齐可在本地重复运行的生产烟测：关键页面/API smoke、性能预算、SEO、PWA/offline、错误体验、邮件 dry-run 与 Stripe 本地校验；并在 2026-05-19 发布窗口完成 live 验收：Vercel Production redeploy、Resend 真实发信、Stripe live $1 支付与退款、Supabase remote check 均通过。P2-D 已补强 AI 安全与成本控制：promptGuard 红队测试、Coach 成本保护、Sentry/PostHog 隐私审计。P2-E 已补齐发布材料：`LAUNCH_CHECKLIST`、README polish、英文 case study、收入实验计划、用户访谈脚本和 30/60/90 天 roadmap。
...
| P2-E | 发布材料       | 已完成首轮落地：LAUNCH_CHECKLIST、GitHub 展示、英文 case study、收入实验计划、用户访谈脚本、30/60/90 天 roadmap |

## Phase 3 首轮完成后的剩余动作

以下动作仍需要 Frank 单独批准：`git push`、创建/更新 PR、创建 GitHub release、DNS/Cloudflare 修改、Supabase 生产变更、公开发布公告、面向外部用户的邮件或营销触达。生产发布窗口已完成的 Resend、Stripe、Vercel 配置保留 24-48 小时观察期，稳定后再清理旧 key。
...
- 不推送、不公开发布、不改生产外部系统、不暴露 secrets，除非 Frank 明确批准。
