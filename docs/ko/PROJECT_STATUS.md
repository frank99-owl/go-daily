...
- **Upstash Redis 속도 제한**: 프로덕션에서는 Upstash Redis로 인스턴스 간 속도 제한을 적용한다. `NODE_ENV === "production"`인데 `UPSTASH_REDIS_REST_URL` / `UPSTASH_REDIS_REST_TOKEN`이 없으면 `createRateLimiter()`가 스텁을 반환하고 첫 `isLimited()` 호출에서 오류를 던진다(`next build`는 Upstash 없이 가능. 개발에서는 둘 다 생략하고 `MemoryRateLimiter` 사용).
...
- **테스트 스위트**: 82개 테스트 파일, 658개 테스트 케이스.
