...
- **코치 완성도**: 코치 데이터 파일은 `coachBasicEligibleIds.json` (3033), `coachReadyIds.json` (3030) 및 `variationGroups.json` (0개 그룹)으로 분할되었으며 `getCoachAccess()`가 데이터 레이어와 런타임 품질 관문을 모두 검증합니다. P0-D는 20개 문제에 대해 `solutionSequence` 및 `wrongBranches`를 보강했고, 최근 백필로 3010개를 추가했습니다. 나머지 **3**개 문제는 단순 텍스트 해설이 있다고 해서 "완전한 AI 코치 문제"로 간주해서는 안 됩니다.
...
1. **품질 등급 명확화**: 문제를 `basic-explained`, `coach-eligible`, `coach-ready` 및 `variation-ready` 등급으로 명확히 구분합니다. 현재 전체 데이터베이스가 `coach-eligible` 기초 후보군으로 기능하며, P0-D의 초기 20개 문제와 백필된 3010개 문제가 `coach-ready`로 승인되었습니다. 나머지 3개 문제는 정답 수순과 오답 분기가 부족합니다(복수 정답 수순이 존재하기 때문).
