...
- 본문 **8 KB**, 동일 출처, IP 제한(`createRateLimiter`: **프로덕션**은 Upstash 필수, **개발**만 인메모리), `guardUserMessage`, `sanitizeInput`, `coachBasicEligibleIds.json` / `coachReadyIds.json` / `variationGroups.json` 및 `checkCoachEligibility` / `getCoachAccess` 런타임 검사, 할당량(Postgres RPC — DATABASE 문서 참고). 게스트 기기별 행은 `service_role`만. **게스트 IP 일일 상한**(`checkIpLimit`, `GUEST_IP_DAILY_LIMIT`, 현재 IP당 UTC 일 **20**회)은 Upstash 설정 시 Redis, 아니면 `guestCoachUsage.ts` 인메모리 `Map`.
...
Today 페이지의 랜덤 연습 액션에서 사용할 임의의 연습 문제를 선택합니다.
...
두 필드는 모두 선택 사항입니다. `attemptedPuzzleIds`는 현재 브라우저에서 아직 풀지 않은 문제를 우선하기 위해 사용합니다. `level`은 onboarding 난이도 구간으로 문제 풀을 필터링하며, 없거나 유효하지 않으면 `beginner`로 되돌아갑니다.
...
임의로 선택된 퍼즐의 ID를 반환합니다. 요청한 구간의 모든 문제를 이미 풀었다면 해당 구간의 전체 문제 풀로 되돌아갑니다. 클라이언트는 이후 별도로 전체 퍼즐 데이터를 조회합니다.
...
- `checkout.session.completed` 이후 Pro 활성화 / 체험 시작 서비스 메일을 발송합니다.
...
인증된 사용자가 Onboarding에서 확정한 난이도 구간을 `profiles.training_level`에 저장하여, 다른 기기에서도 오늘의 문제가 같은 레벨 구간을 기준으로 출제되도록 합니다.
...
**동작**: `GET`은 이메일 하단의 보이는 링크이며 `profiles.email_opt_out = true`를 저장한 뒤 현지화된 확인 페이지를 반환합니다. `POST`는 메일 클라이언트가 `List-Unsubscribe` 및 `List-Unsubscribe-Post` 헤더로 호출하는 RFC 8058 원클릭 수신 거부이며, `profiles.email_opt_out = true`를 저장한 뒤 빈 응답을 반환합니다.
...
`/admin` Operations Snapshot에서 사용하는 읽기 전용 운영 요약을 반환합니다: 콘텐츠 레이어, 최근 30일 코치 사용량, Stripe 구독 상태 및 `past_due` 유예 상태, 최근 Stripe 웹훅 오류 발생 여부, 최근 7일 동기화 쓰기 징후.

**인증**: 세션 `user.id`가 `ADMIN_USER_IDS`에 포함되어야 합니다.

**응답** (`200`): `{ "content", "coach", "stripe", "webhooks", "sync", "generatedAt" }`. 응답에는 사용자 이메일, 원본 메시지 본문, 비밀키 또는 비공개 로그가 포함되지 않습니다.
