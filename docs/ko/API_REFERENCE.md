...
**인증**: 선택 사항. 로그인 사용자는 Supabase 세션 쿠키를 사용합니다. 게스트는 헤더 `x-go-daily-guest-device-id`가 필요합니다(더 낮은 할당량).

**요청 본문** (JSON, `CoachRequestSchema`로 유효성 검증):

**참고**: 히스토리 메시지는 메시지별 2,000자 절단 외에도 총 6,000자의 문자 예산 제한을 받습니다 (최신 메시지부터 절단).
...
| 401 | `login_required` | 세션과 게스트 헤더가 모두 없음 |
...
- 게스트 사용량은 Supabase `guest_coach_usage`에 `service_role`로 저장; IP 한도는 인메모리(`guestCoachUsage.ts`)

### `GET /api/coach`

현재 호출자의 코치 사용량 요약을 조회합니다.

**인증**: 선택 사항. 로그인 사용자는 세션 쿠키를 사용합니다. 게스트는 `x-go-daily-guest-device-id`로 게스트 할당량을 조회할 수 있습니다. 둘 다 없으면 `401`.
...
두 리미터 모두 최대 엔트리 수를 강제하며 (`MemoryRateLimiter`는 50,000개), 만료된 엔트리를 제거하여 메모리 무한 증가를 방지합니다.
