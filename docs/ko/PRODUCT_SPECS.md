...
시스템은 '콘텐츠 우선' 법적 제공 메커니즘을 채택하고 있습니다.

- **동적 법적 푸터**: 푸터는 활성화된 로케일에 따라 `/legal/[kind]` 경로로 연결됩니다.
- **관할 구역별 특정 라우트**:
  - `/ja/legal/tokushoho`: 일본 시장의 '특정상거래법에 따른 표시' 의무를 충족하기 위해 필수적으로 운용됩니다.
  - **한국 PIPA 브릿지 (계획 중)**: 한국 로케일 사용자를 대상으로 싱가포르 및 미국으로의 데이터 이전에 대한 명시적 동의를 얻기 위한 차단형 모달을 구현할 예정입니다.
- **약관 동의**: 회원가입은 서비스 이용약관 및 개인정보 처리방침에 대한 동의를 내포합니다. 결제(Checkout) 프로세스에는 디지털 상품 환불 정책에 대한 2차 확인 절차가 포함됩니다.
