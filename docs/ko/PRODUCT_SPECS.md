...
우리는 개선된 SuperMemo-2 (SM-2) 알고리즘을 구현했으며, 오답 원인 기반 품질 매핑을 추가했습니다.

- **초기 상태**: Ease Factor 2.5, Interval 0. Ease Factor 최솟값은 1.3.
- **품질 매핑** (`qualityFromAttemptWithReason` 기준):
  - 정답 (어떤 오답 원인이든) -> 5
  - 오답, `missed-vital-point` (근접 실수) -> 3 (경미한 페널티; SM-2에서 "통과"로 처리, 간격은 1일로 진행)
  - 오답, `shape-reading` 또는 `liberty-counting` (핵심 계산 오류) -> 1 (강한 페널티; Ease Factor가 크게 하락, 간격은 0 유지)
  - 오답, `endgame-value`, `opening-direction`, 또는 알 수 없음 -> 2 (표준 페널티; 즉시 재대기 트리거)
- **Ease Factor 갱신**: `nextEase = currentEaseFactor + (0.1 - (5 - q) * (0.08 + (5 - q) * 0.02))`, 최솟값 1.3으로 클램핑.
- **간격 진행** (품질 >= 3): 0 -> 1일 -> 6일 -> 이후 `ceil(이전 간격 * nextEase)`.
- **품질 < 3**: 간격을 0으로 리셋 (같은 날 즉시 재대기).
- **스케줄링**: 문제는 `due_date` 오름차순으로 정렬됩니다. 복습 목록은 약점 일치 우선 정렬됩니다(사용자의 빈번한 오답 원인에 해당하는 문제 먼저 표시). Pro 사용자는 백로그를 정리하여 오답 관리의 "인박스 제로(Inbox Zero)"를 달성할 수 있습니다.

### 오답 원인 분류 (`lib/puzzle/mistakeReason.ts`)

시스템은 퍼즐 태그, 난이도, 사용자 착수 위치를 기반으로 오답 원인을 자동 분류합니다 (5가지 유형):

| 오답 원인 ID         | 분류 로직                                                        | 의미                         |
| -------------------- | ---------------------------------------------------------------- | ---------------------------- |
| `missed-vital-point` | 사용자 착수가 정답으로부터 체비셰프 거리 <= 1                     | 근접 실수, 거의 정답         |
| `shape-reading`      | 수법 또는 고난이도 (>= 4) 사활에서 정답과 먼 착수                | 국면 형상 판단 오류          |
| `liberty-counting`   | 저난이도 (<= 3) 사활에서 정답과 먼 착수                          | 팻수 계산 오류               |
| `endgame-value`      | 퍼즐 태그가 `endgame`                                            | 끝내기 가치 판단 오류        |
| `opening-direction`  | 퍼즐 태그가 `opening`                                            | 포석 방향 선택 오류          |

오답 원인 데이터는 3개 시스템에서 활용됩니다: SRS 품질 매핑 (복습 간격 영향), 추천 엔진 (약점 타겟팅), 훈련 통계 대시보드 (취약점 표시).
...
| Next recommendation | 단순 무작위가 아닌 최적의 다음 문제                             | 난이도, 태그, SRS만료, 최근 오답, 오답 원인, 품질 등급, 적응형 정확도 |

**추천 엔진 로직** (`lib/puzzle/nextRecommendation.ts`): 엔진은 주요 액션(`continue-practice` 또는 `review-mistakes`), 적응형 난이도 힌트(`same-level`, `step-up`, `step-down`), 그리고 이유 ID를 기반으로 다음 문제를 선택합니다. 주요 동작:

- 온보딩: 선택한 레벨 유지.
- 이전 기록 없음: 현재 난이도로 폴백.
- 복습 백로그 >= 5: `review-mistakes` 우선.
- 오답: 동일 테마 및 오답 원인 타겟팅.
- 정답 + 약점 있음: 가장 약한 오답 원인 카테고리 타겟팅 (현재 태그와 다른 경우).
- 정답 + 최근 정확도 >= 80% (최근 10문제): 난이도 상승.
- 정답 + 최근 정확도 <= 40% (최근 10문제): 난이도 하락.
- 기본값: 현재 문제에 이전 오답이 없으면 보수적으로 한 단계 상승.
