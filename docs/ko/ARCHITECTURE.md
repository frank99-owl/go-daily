...
이 문서는 go-daily의 내부 구조, `lib/` 디렉토리의 "9개 도메인 분리" 리팩토링 및 중앙 집중식 미들웨어 로직에 대해 설명합니다.

## 1. 글로벌 요청 생명주기 (`proxy.ts`)

모든 사용자 대상 요청은 `proxy.ts` 미들웨어(Next.js 15 규약에 따라 `middleware` 내보내기)를 통과합니다. 이 미들웨어는 단일 패스로 다음과 같은 네 가지 핵심 작업을 처리합니다:

1.  **매니페스트 특수 처리 (Manifest Special-Case)**: PWA 매니페스트 요청을 가로채어 적절한 버전을 반환합니다.
2.  **면제 경로 통과 (Exempt Path Passthrough)**: 특정 경로(정적 자산, API 웹훅 등)가 모든 미들웨어 로직을 우회하도록 허용합니다.
...
- **핵심 엔진**: 돌 배치, 규칙 적용(호흡점, 사석, 코), 바둑판 표시를 6개의 소스 파일로 구현.
...
- **서비스 격리**: `proxy.ts` 미들웨어(`middleware` 내보내기)를 통해 인증 및 권한이 부여된 요청만이 핵심 API 라우트(Stripe/Coach)에 도달하도록 보장합니다.
