...
3. **在 `content/curatedPuzzles.ts` 加一条**：
...
数据入口层（`content/puzzles.ts` / `content/puzzles.server.ts`）会自动将 `curatedPuzzles.ts` 与 `importedPuzzles.json` 聚合。你不需要手动 merge。
...
当前的 `content/data/importedPuzzles.json`（100 题 Cho Chikun 初级死活）就是这样来的。
...
3. 写入 `content/data/importedPuzzles.json`（不要手改）
...
2. 写到独立文件 `content/data/importedX.json`（不要手改约定）
3. 在 `content/puzzles.server.ts` 里注册新数据源：
...
- 直接往 `content/data/importedPuzzles.json` 里追加别的源的题 —— 这个文件是 auto-generated 的
- 跳过校验直接 commit —— 你一个人 work 的时候可能没事，但 CI/部署会当场炸

## 校验规则清单

`scripts/validatePuzzles.ts` 先用 `zod` schema 做基础类型校验，再用自定义规则做语义检查：
...
| 新 curated 题不出现在 `/puzzles` 列表   | 检查 `content/puzzles.server.ts` 是否正确聚合了 curated 数据源；重启 dev server              |
