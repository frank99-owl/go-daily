...
当前的 `content/data/importedPuzzles.ts`（100 题 Cho Chikun 初级死活）就是这样来的。
...
3. 写入 `content/data/importedPuzzles.ts`（带 auto-generated 横幅，不要手改）
...
2. 写到独立文件 `content/data/importedX.ts`（auto-generated 横幅 + 不手改约定）
...
- 直接往 `content/data/importedPuzzles.ts` 里追加别的源的题 —— 这个文件是 auto-generated 的
