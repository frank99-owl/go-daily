...
改良された SuperMemo-2 (SM-2) アルゴリズムを実装し、誤答理由別の品質マッピングを追加しています。

- **初期状態**： Ease Factor 2.5, Interval 0。Ease Factor の下限は 1.3。
- **品質マッピング**（`qualityFromAttemptWithReason` 基準）：
  - 正解（理由問わず） -> 5
  - 不正解、`missed-vital-point`（ニアミス） -> 3（軽いペナルティ；SM-2 では「合格」と扱い、インターバルは 1 日に進行）
  - 不正解、`shape-reading` または `liberty-counting`（基本計算エラー） -> 1（重いペナルティ；Ease Factor の低下がより大きく、インターバルは 0 に留まる）
  - 不正解、`endgame-value`、`opening-direction`、または不明 -> 2（標準ペナルティ；即時再キューをトリガー）
- **Ease Factor 更新**： `nextEase = currentEaseFactor + (0.1 - (5 - q) * (0.08 + (5 - q) * 0.02))`、最小値 1.3 にクランプ。
- **インターバル進行**（品質 >= 3）： 0 -> 1 日 -> 6 日 -> その後 `ceil(前回インターバル * nextEase)`。
- **品質 < 3**： インターバルを 0 にリセット（同日即時再キュー）。
- **スケジューリング**： 問題は `due_date` の昇順で並べられます。復習リストは弱項目一致で優先ソートされます（ユーザーの高頻度誤答理由に該当する問題が先に表示）。Pro ユーザーはバックログをクリアして、ミス問題管理の「インボックス・ゼロ」を達成できます。

### 誤答理由の分類 (`lib/puzzle/mistakeReason.ts`)

システムはパズルのタグ、難易度、ユーザーの着手位置に基づき、誤答理由を自動分類します（5 種類）：

| 誤答理由 ID          | 分類ロジック                                           | 意味                 |
| -------------------- | ------------------------------------------------------ | -------------------- |
| `missed-vital-point` | ユーザーの着手が正解からチェビシェフ距離 <= 1          | ニアミス、ほぼ正解   |
| `shape-reading`      | 手筋または高難易度 (>= 4) の死活で、正解から離れた着手 | 局面の形の読み間違い |
| `liberty-counting`   | 低難易度 (<= 3) の死活で、正解から離れた着手           | 呼び気の計算エラー   |
| `endgame-value`      | パズルのタグが `endgame`                               | 官子の価値判断エラー |
| `opening-direction`  | パズルのタグが `opening`                               | 布石の方向選択エラー |

誤答理由データは 3 つのシステムで活用されます：SRS 品質マッピング（復習間隔への影響）、推薦エンジン（弱項目ターゲティング）、訓練統計パネル（弱点表示）。
...
| Next recommendation | 単なるランダムではなく、最適な次の問題                             | 難易度, タグ, SRS期限, 最近の誤答, 誤答理由, 品質層, 適応的正答率 |

**推薦エンジンのロジック**（`lib/puzzle/nextRecommendation.ts`）：エンジンは主アクション（`continue-practice` または `review-mistakes`）、適応的難易度ヒント（`same-level`、`step-up`、`step-down`）、および理由 ID に基づいて次の問題を選択します。主要な動作：

- オンボーディング：選択されたレベルを維持。
- 過去の記録なし：現在の難易度にフォールバック。
- 復習バックログ >= 5：`review-mistakes` を優先。
- 不正解：同じテーマと誤答理由をターゲット。
- 正解で弱項目あり：最弱の誤答理由カテゴリをターゲット（現在のタグと異なる場合）。
- 正解で最近の正答率 >= 80%（直近 10 問）：難易度を一段階上げる。
- 正解で最近の正答率 <= 40%（直近 10 問）：難易度を一段階下げる。
- デフォルト：現在の問題に過去の不正解がない場合、保守的に一段階上げる。
