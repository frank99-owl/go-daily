...
Today ページ内のランダム練習アクションで使うランダム練習問題を選択する。
...
どちらのフィールドも任意。`attemptedPuzzleIds` は現在のブラウザで未挑戦の問題を優先するために使う。`level` は onboarding の難度帯で問題プールを絞り込み、欠落または無効な値は `beginner` にフォールバックする。
...
ランダムに選択されたパズルの ID を返す。指定した帯の問題をすべて試行済みの場合は、その帯の全問題プールにフォールバックする。クライアントはその後、別途フルデータを取得する。
...
- `checkout.session.completed` 後に Pro 有効化 / トライアル開始のサービスメールを送信する。
...
認証済みユーザーが Onboarding で確定した難易度帯を `profiles.training_level` に保存し、別端末でも「今日の問題」が同じレベル帯で出題されるようにする。
...
**挙動**: `GET` はメール本文下部の表示リンクで、`profiles.email_opt_out = true` を設定した後、ローカライズされた確認ページを返します。`POST` はメールクライアントが `List-Unsubscribe` / `List-Unsubscribe-Post` ヘッダーから実行する RFC 8058 のワンクリック配信停止で、`profiles.email_opt_out = true` を設定して空レスポンスを返します。
...
`/admin` の Operations Snapshot で使用される読み取り専用の運用サマリーを返します：コンテンツ層、過去30日間のコーチ使用状況、Stripeの購読状態および `past_due` の猶予状況、最近の Stripe webhook の問題、過去7日間の同期書き込みの兆候。

**認証**: セッション `user.id` が `ADMIN_USER_IDS` に含まれていること。

**レスポンス** (`200`): `{ "content", "coach", "stripe", "webhooks", "sync", "generatedAt" }`。レスポンスには、ユーザーのメールアドレス、生メッセージ本文、シークレット、プライベートログは含まれません。
