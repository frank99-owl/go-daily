...
| Imported corpus (generated) | `lib/importedPuzzles.ts` | Produced by `scripts/importTsumego.ts` — do not hand-edit |
...
| Localized text | `lib/localized.ts` | `localized(text, locale)` with en→zh→ja→ko fallback |
...
| `npm run import:puzzles` | Pulls the top 100 problems from sanderland/tsumego into `lib/importedPuzzles.ts` |
