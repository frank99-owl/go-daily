...
**登录用户**（`lib/syncStorage.ts`）：

- 本地立即写入 `localStorage`
- 同时入队 `IndexedDB` 队列
- 后台 flush 到 Supabase（指数退避重试，最大 10 次）
- 页面初始化时 `sync()` 拉取云端增量合并到本地
...
已接入 Supabase Auth + Postgres：

- 用户登录后自动 `registerDevice()`（`lib/deviceRegistry.ts`）
- `planMerge()` + `applyMergeDecision()` 处理 local vs remote 冲突（`lib/mergeOnLogin.ts`）
- `syncStorage.ts` 双写 + 队列 + 重试
- 免费版限 1 台设备，付费版不限
...
export async function generateStaticParams() {
  // 仅 curated 题 SSG，其余 ISR
  return CURATED_PUZZLES.map((p) => ({ id: p.id }));
}
export const revalidate = 86400;
```

1 000 题 → 构建时生成约 300 个静态页，可接受。
10 000 题 → 需评估构建时间，必要时全部改用 ISR（fallback: "blocking"）。
...
| 观测     | PostHog + Sentry + Vercel Analytics + Speed Insights 已接入         |
...
- **新增语言**：在 `content/messages/` 新增 JSON，在 `lib/localePath.ts` 的 `SUPPORTED_LOCALES` 中注册，在 `types/index.ts` 的 `Locale` 中添加，在 `lib/i18n.tsx` 的 `DICTS` 中注册。
...
- [ ] 多实例部署：速率限制改用 Redis
- [x] AI 教练模型：`COACH_MODEL` 环境变量已接入
- [x] 跨设备同步需求：Supabase Auth + syncStorage 已接入
- [x] 用户认证：Supabase OAuth 已接入
- [x] 分析观测：PostHog + Sentry 已接入
- [x] 设备限制 + 付费墙：deviceRegistry 已接入
