...
- **Coach Completeness**: Coach data files are split into `coachBasicEligibleIds.json` (3033), `coachReadyIds.json` (3030), and `variationGroups.json` (0 groups); `getCoachAccess()` layers these with runtime checks. P0-D completed the integration of `solutionSequence` and `wrongBranches` for 20 puzzles, and recent backfills added 3010 more; the remaining **3** puzzles should not be considered "full AI Coach puzzles" merely because they have text explanations.
...
1. **Establish Tiered Facts**: Explicitly categorize puzzles into `basic-explained`, `coach-eligible`, `coach-ready`, and `variation-ready` tiers. Currently, the entire database serves as `coach-eligible` candidates; the initial 20 puzzles in P0-D and 3010 backfilled puzzles are approved as `coach-ready`. The remaining 3 puzzles lack target sequences and wrong branches (due to multiple correct paths).
