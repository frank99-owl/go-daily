...
```
┌─────────────────────────────────────────────────────────────┐
│  proxy.ts       请求转发（i18n cookie → header）              │
...
| `/today`        | `app/today/page.tsx`        | `app/TodayClient.tsx`              | 每日一题：异步 `getPuzzleForDate(todayLocalKey())`，交给 `TodayClient` 做交互 |
| `/puzzles`      | `app/puzzles/page.tsx`      | `app/puzzles/PuzzleListClient.tsx` | 全量题库：异步获取 summary 列表，筛选 / 排序 / 搜索                           |
| `/puzzles/[id]` | `app/puzzles/[id]/page.tsx` | 复用 `TodayClient`                 | 按 ID 打开一题；`generateStaticParams()` 覆盖全量题库 ID                      |
...
| `/api/coach`    | `app/api/coach/route.ts`    | —                                  | LLM 代理（POST JSON，zod schema 校验）                                        |

**Server vs Client 分界约定**：`page.tsx` 尽量只做「异步取 puzzle · 构造 props · 传给 Client 组件」，重活都在 `*Client.tsx` 里。因为需要读 `localStorage`，所有带历史/偏好的交互都必然是 client 组件。
...
> `getPuzzle(id)` 是 `content/puzzles.ts` 导出的 async 函数：服务端走 `puzzles.server.ts` 读全量数据，客户端走 `puzzleIndex.json` 做轻量查询。
...
| 导入题集（自动生成） | `content/data/importedPuzzles.json` | `scripts/importTsumego.ts` 产出，不要手改                                |
| 全量题库（自动生成） | `content/data/puzzleLibrary.json`   | 聚合后的完整题库 JSON                                                    |
...
| i18n proxy           | `proxy.ts`                          | cookie `go-daily.locale` → `x-locale` header，消除 SSR 语言闪烁          |
...
| `npm run import:puzzles`   | 拉取 sanderland/tsumego 前 100 题，写入 `content/data/importedPuzzles.json`              |
...
| `npm run test`             | Vitest 单元测试（board / judge / goRules / sgf / puzzleOfTheDay / storage + API + 组件） |

**关键设计**：`prebuild` → `validate:puzzles` 是一层「部署保险」。任何让站点 404 或 crash 的脏数据都会在 `npm run build` 里当场爆，不会上线。校验器现在先用 `zod` schema 做第一层类型检查，再用自定义规则做语义检查。
