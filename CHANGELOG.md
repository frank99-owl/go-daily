...
- **Dependency and security refresh (2026-08-24)**: Next 16.2.10 → 16.3.2, clearing nine advisories including a middleware/proxy bypass in App Router apps (this app routes every request through `proxy.ts`), SSRF in rewrites, and unauthenticated disclosure of internal Server Function endpoints. Both `overrides` pins were themselves inside vulnerable ranges (`postcss` 8.5.14 ≤ 8.5.22, `fast-uri` 3.1.2 within 3.0.0–3.1.4) and were raised to 8.5.26 and 3.1.6. Remaining in-range minor/patch updates applied; `npm audit` reports zero vulnerabilities, unblocking CI, which had failed `npm audit --audit-level=high` since 2026-08-01. Stripe is deliberately held at 22.3.0: 22.5.0 retypes the pinned `apiVersion` to `2026-07-29.dahlia`, which changes the API version applied to live checkout, portal, and webhook traffic. Test count: **111 files / 989 cases**.
