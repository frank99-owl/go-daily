...
- **Docs aligned with code (2026-05)**: Vitest inventory (82 files / 658 tests), CI Node 22, README `engines` (Node 22.5+), `proxy.ts` scope vs `/api` bypass and route-level auth, entitlements modules split out from `lib/puzzle/`, Upstash required in production for `createRateLimiter`, `MemoryRateLimiter` eviction semantics, preflight checklist described as variable (see `productionPreflight.ts`), multilingual `ARCHITECTURE` / `OPERATIONS_QA` / `API_REFERENCE` / `PROJECT_STATUS` updates.
