...
export function PIPAConsentCard({ onAccept }: { onAccept: () => void }) {
  ...
  return (
    <div className="relative flex flex-col gap-8 rounded-2xl border border-white/10 bg-white/[0.02] p-8 backdrop-blur sm:p-10">
      ...
      <button
        onClick={onAccept}
        className="group relative flex h-14 w-full items-center justify-center overflow-hidden rounded-full bg-white text-sm font-semibold text-black transition-all hover:bg-[#00f2ff] hover:text-black hover:shadow-[0_0_30px_rgba(0,242,255,0.3)]"
      >
